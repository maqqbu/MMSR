clc
clear all
crowd = load("../original_datasets/trec_crowd.txt");
truth = load("../original_datasets/trec_truth.txt");
task_ids = crowd(:, 1);
worker_ids = crowd(:, 2);
class_ids = crowd(:, 3);
truth_ids = truth(:, 1);
truth_collection = truth(:, 2);
num_class = length(unique(class_ids));
num_tasks = length(unique(task_ids));
num_worker = length(unique(worker_ids));
% collect the ground_truth vector (some of the true label are missing, the true label are not repeating and 
% is collected in ascending order)
ground_truth = zeros(size(unique(truth_ids)));
for i = 1 : length(truth_ids)
    ground_truth(i) = truth_collection(i);
end
data = crowd;
miss_idx = [];
for i = 1 : num_tasks
...
sparsity = min(sum(sign(N_original)));
graph_sparsity = sum(sum(sign(N_original)))/num_worker^2;
Y_sparsity = sum(sum(sign(Y2)))/(num_tasks * num_worker);
...
save_name = ['../corrupted_datasets/trec/trec_raw_corruption_', num2str(F),'_', num2str(times), '.mat'];
