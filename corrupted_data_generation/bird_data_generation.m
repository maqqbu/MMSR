clc
clear all
crowd = load("../original_datasets/bluebird_crowd.txt");
truth = load("../original_datasets/bluebird_truth.txt");
task_ids = crowd(:, 1);
worker_ids = crowd(:, 2);
class_ids = crowd(:, 3);
truth_ids = truth(:, 1);
...
% collect the ground_truth vector
ground_truth = zeros(num_tasks, 1);
for i = 1 : length(truth_ids)
    ground_truth(truth_ids(i)) = truth_collection(i);
end
data = crowd;
miss_idx = [];
for i = 1 : num_tasks
...
A = abs(sign(Y_obs)); % the positions of valid observed entries
...
for F = 0 : 2 : 20
    ...
    for times = 1 : 3
        ...
        save_name = ['../corrupted_datasets/bird/bird_39_corruption_', num2str(F),'_', num2str(times), '.mat'];
