clc
clear all
crowd = load("../original_datasets/rte_crowd.txt");
truth = load("../original_datasets/rte_truth.txt");
task_ids = crowd(:, 1);
worker_ids = crowd(:, 2);
class_ids = crowd(:, 3);
truth_ids = truth(:, 1);
...
Y_sparsity = sum(sum(sign(Y2)))/(num_worker * num_tasks);
...
for F = 0 : 8 : 80
    ...
    for times = 1 : 3
        ...
        save_name = ['../corrupted_datasets/RTE/RTE_raw_corruption_', num2str(F),'_', num2str(times), '.mat'];
